...
if page == "📊 Market Dashboard":
    ...
    with st.sidebar:
        ...
        custom_price_columns = [
            "spread_4h_Real_Time", "spread_4h_Day_Ahead", "spread_4h_Pre_Dispatch", "spread_Day_Ahead_vs_Real_Time"
        ]
    ...
    def melt_df(df_to_melt):
        ...
        melted["Market"] = price_type if price_type in custom_price_columns else melted["Market"].str.extract(r"_(.*)$")[0]
        return melted
        ...
        for market in df_melted["Market"].unique():
            for location in df_melted["Pricing Location"].unique():
                ...
                fig.add_trace(go.Scatter(
                    ...
                    mode="lines+markers", name=f"{location} – {market}", line=dict(dash="solid")
                ))
    ...
    if df_avg_melted is not None and not df_avg_melted.empty:
        ...
        for market in df_avg_melted["Market"].unique():
            ...
            fig.add_trace(go.Scatter(
                x=subset["timestamp"], y=subset["Price"],
                mode="lines+markers", name=f"Average – {market}", line=dict(dash="dot", width=3)
            ))

    fig.update_layout(
        title=f"{price_labels.get(price_type, price_type)} per {'interval' if hourly_bool else 'hour'} across markets",
        xaxis_title="Timestamp", yaxis_title="Price ($/MWh)",
        legend_title="Zone / Market", template="plotly_white",
...
# ======= STATISTICS PAGE =======
    ...
    with col2:
        metric = st.selectbox("Choose metric", [
            "LMP_Day_Ahead", "LMP_Real_Time",
            "spread_4h_Day_Ahead", "spread_4h_Real_Time",
