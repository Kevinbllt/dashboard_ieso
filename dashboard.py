...
from src.filter import apply_filters, compute_average_by_hour
...
if dataset_option == 'Energy - Hourly':
    df = pd.read_csv("data/energy/processed/energy_hourly_historical.csv", parse_dates=["Date"])

elif dataset_option == 'Operating Reserve - Hourly':
    df = pd.read_csv("data/operating_reserve/processed/operating_reserve_hourly_historical.csv", parse_dates=["Date"])

elif dataset_option == 'Energy - 5-min intervals':
    df = pd.read_csv("data/energy/processed/energy_interval_historical.csv", parse_dates=["Date"])

elif dataset_option == 'Operating Reserve - 5-min intervals':
    df = pd.read_csv("data/operating_reserve/processed/operating_reserve_interval_historical.csv", parse_dates=["Date"])
